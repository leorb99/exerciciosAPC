...
with open('carteira.txt', 'r') as arq:
    total = 0
    for linha in arq:
        conteudo = linha.strip('\n')
        conteudo = conteudo.split('/')
        carteira[conteudo[0]] = float(conteudo[1])
        total += carteira[conteudo[0]]

if carteira['Dinheiro'] < valor <= carteira['Cartão de crédito']:
    print('O jeito é pagar com o cartão de crédito')
elif valor <= carteira['Dinheiro'] and nao_tem == True:
    print('Vou pagar com dinheiro')
elif valor > (carteira['Dinheiro'] + carteira['Cartão de crédito']):
